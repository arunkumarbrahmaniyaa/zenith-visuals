...
export interface TreeProps extends HierarchyChartProps {
  /** Layout direction. Default "vertical" (root at top). */
  orientation?: "vertical" | "horizontal";
  /** Node marker radius in px. Default 5. */
  nodeRadius?: number;
...
/**
 * Tidy node-link tree — a classic hierarchy diagram with curved links. Leaves
 * are spread evenly and parents centered over children. Responsive and SSR-safe.
 *
 * @example
 * <Tree data={{ name: "CEO", children: [{ name: "CTO" }, { name: "CFO" }] }} />
 */
        ...
        return (
          <>
            <svg width={width} height={h} viewBox={`0 0 ${width} ${h}`} role="img"
              aria-label={base.labels?.ariaLabel ?? "Tree diagram"} style={{ display: "block" }}>
              <g>
                {nodes.map((node) =>
